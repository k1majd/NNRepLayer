...
def repair_weights(
    model_orig, layer_to_repair, architecture, x_train, y_train, cost_function_weights
):
    """_summary_
    ...
    Args:
        ...
        cost_function_weights (_type_): _description_
    ...
    y_ = mip_model_layer(
        layer_values_train[layer_to_repair - 2],
        (num_samples, architecture[layer_to_repair - 1]),
        w_b_bound_error=1,
    )
    ...
    new_weight = np.zeros(
        (architecture[layer_to_repair - 1], architecture[layer_to_repair])
    )
