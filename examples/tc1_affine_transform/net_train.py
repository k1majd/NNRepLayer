...
def arg_parser():
    ...
    parser.add_argument('-ep', '--epoch', nargs = '?', type = int, const = 1000, default = 1000, help = 'Specify training epochs in int')
...
def main(direc, learning_rate, regularizer_rate, train_epochs, visual):
    ...
    translate2 = np.array([[1, 0, -2.5], [0, 1, -2.5], [0, 0, 1]])  # translation matrix 2
    rotate = np.array([[np.cos(np.pi / 4), -np.sin(np.pi / 4), 0], [np.sin(np.pi / 4), np.cos(np.pi / 4), 0], [0, 0, 1]])  # rotation matrix
    ...
    vert_const_inp = np.array([[1.25, 3.75, 3.75, 1.25],[1.25, 1.25, 3.75, 3.75],[1, 1 , 1, 1]]) # contraint vertices in input space
    vert_const_out = np.matmul(np.matmul(np.matmul(translate1, rotate), translate2), vert_const_inp) # constraint vertices in output space
    poly_const = Polygon([(vert_const_out[0, 0], vert_const_out[1, 0]), (vert_const_out[0, 1], vert_const_out[1, 1]), (vert_const_out[0, 2], vert_const_out[1, 2]), (vert_const_out[0, 3], vert_const_out[1, 3])])
    ## Network
    input_dim = 3
    output_dim = 3
    hid_dim_0 = 10
    ...
    his = model_orig.fit(x_train, y_train, epochs = train_epochs, use_multiprocessing = True, verbose = 0)
    ...
    if visual == 1:
        ...
        plt.plot(x_poly_orig_bound, y_poly_orig_bound, color='plum', alpha=0.7, linewidth=3, solid_capstyle='round', zorder=2, label='Original Set')
        plt.plot(x_poly_trans_bound, y_poly_trans_bound, color='tab:blue', alpha=0.7, linewidth=3, solid_capstyle='round', zorder=2, label='Target Set')
        plt.scatter(y_train[:, 0], y_train[:, 1], color = 'tab:blue', label='Original Target')
        y_predict_train = model_orig.predict(x_train)
        plt.scatter(y_predict_train[:, 0], y_predict_train[:, 1], color = 'mediumseagreen', label='Predicted Target')
        ...
        plt.plot(x_poly_orig_bound, y_poly_orig_bound, color='plum', alpha=0.7, linewidth=3, solid_capstyle='round', zorder=2, label='Original Set')
        plt.plot(x_poly_trans_bound, y_poly_trans_bound, color='tab:blue', alpha=0.7, linewidth=3, solid_capstyle='round', zorder=2, label='Target Set')
        plt.scatter(y_test[:, 0], y_test[:, 1], color = 'tab:blue', label='Original Target')
        y_predict_test = model_orig.predict(x_test)
        plt.scatter(y_predict_test[:, 0], y_predict_test[:, 1], color = 'mediumseagreen', label='Predicted Target')
        plt.legend(loc="upper left",frameon=False,fontsize=20)
