...
def hand_label_samples(x_train, y_train, model, box):
    ...
    for i in range(x_train.shape[0]):
        point = np.array(
            [x_train[i, -5] + x_train[i, -4], x_train[i, -1] + y_pred[i][0]]
        )
        if is_in_box(point, box):
            idx = np.argmin(np.abs(box[2:] - point[1])) + 2
            y.append(np.array([box[idx] - x_train[i, -1]]))
            x.append(x_train[i])
        else:
            x.append(x_train[i])
            y.append(y_train[i])
...
if __name__ == "__main__":
    ...
    x_train, y_train = hand_label_samples(
        x_repair, y_repair, model_orig, box_inflate
    )
    ...
    callback_reduce_lr = ReduceLROnPlateau(
        monitor="loss", factor=0.2, patience=20, min_lr=0.001
    ...
    sat_rate, mae = give_stats(
        model_orig, test_obs, test_ctrls, ctrl_test_pred_orig, box
    )
    ...
    sat_repair = give_sat_rate(
        model_orig, x_repair, y_repair, ctrl_train_pred_orig, box
    )
