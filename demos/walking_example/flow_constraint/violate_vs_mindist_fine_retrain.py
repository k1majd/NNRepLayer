...
def give_mean_and_upperstd(model, x_train, x_test, box):
    dist = []
    violation = []
    test_pred = model.predict(x_test)
    for i in range(x_test.shape[0]):
        ...
        point = np.array(
            [x_test[i, -5] + x_test[i, -4], x_test[i, -1] + test_pred[i][0]]
        )
        if is_in_box(point, box):
            violation.append((np.min(np.abs(box[2:] - point[1]))))
        else:
            violation.append(0.0)
    ...
    violate_orig = [
        violation_mean[i]
        for i in range(violation_mean.shape[0])
        if not (math.isnan(violation_mean[i]))
    ]
    bin_orig = [
        bins[:-1][i]
    ...
    mean = interp1d(bin_orig, violate_orig, kind="cubic")
    distance = np.linspace(0, np.max(bins[:-1]), 50)

    return mean(distance), mean(distance), distance


def give_dist_n_violate(path_to_folder, num_models, x_test, box):
    means = []
    dists = []
    for i in range(num_models):
        model, x_repair, _ = load_model_n_data(path_to_folder, i + 1)
        _, mean_temp, dist_temp = give_mean_and_upperstd(
            model, x_repair, x_test, box
        )
...
def get_x_y_test(bound):
    ...
    for i in range(x_test.shape[0]):
        point = np.array(
            [x_test[i, -5] + x_test[i, -4], x_test[i, -1] + y_pred[i][0]]
        )
        if is_in_box(point, box):
            idx = np.argmin(np.abs(box[2:] - point[1])) + 2
            y.append(np.array([box[idx] - x_test[i, -1]]))
            x.append(x_test[i])
...
if __name__ == "__main__":
    box = [-2.0, -0.5, 1.0, 3.0]
    path_to_finetune = (
        os.path.dirname(os.path.realpath(__file__)) + "/finetune_model"
    )
    path_to_retrain = (
        os.path.dirname(os.path.realpath(__file__)) + "/retrain_model"
    )
    x_test = get_x_y_test(box)
    mean_fine_tune, dist_fine_tune = give_dist_n_violate(
        path_to_finetune, 50, x_test, box
    )
    mean_retrain, dist_retrain = give_dist_n_violate(
        path_to_retrain, 10, x_test, box
    )
    # mean of mean_fine_tune
    mean_fine_tune = np.mean(mean_fine_tune, axis=0)
    dist_fine_tune = dist_fine_tune[0]
    # mean of mean_retrain
    mean_retrain = np.mean(mean_retrain, axis=0)
    dist_retrain = dist_retrain[0]

    # plot data
    plt.plot(dist_fine_tune, np.abs(mean_fine_tune), "red")
    plt.plot(dist_retrain, np.abs(mean_retrain))
