    ...
    if repair is True:
        for i in range(next_x.shape[0]):
            if is_in_box([next_x[i], next_y[i]], box):
                next_y[i] = box[-2]
    ax1.scatter(next_x, next_y, color=c, alpha=alpha, s=size)
...
if __name__ == "__main__":
    ...
    ax2.fill_between(
        np.linspace(
            0, femure_orig.shape[0], femure_orig.shape[0], endpoint=True
        ),
        0,
        1,
        where=(femure_orig.flatten() >= box[0])
        & (femure_orig.flatten() < box[1]),
        color=color_fill,
        alpha=0.8,
    ...
    ax2.axhline(
        y=box[2], color=color_xline, linewidth=1, linestyle=(0, (5, 7))
    )  # upper bound
    ax2.set_ylabel("Ankle angle (rad)", fontsize=font_size)
    ...
    ax2.set_xlim(xlim)
    ax2.set_ylim(ylim)
    ax2.set_yticks(np.linspace(ylim[0], ylim[1], 3, endpoint=True))
    ax2.set_xticks(np.linspace(xlim[0], xlim[1], 3, endpoint=True))
    ax2.yaxis.set_major_formatter(FormatStrFormatter("%.1f"))
    ax2.xaxis.set_major_formatter(FormatStrFormatter("%.0f"))
    # set ax2 tick fonts with latex

    # ax2.rcParams.update({"text.usetex": True})
    # ax2.yaxis.set_label_position("right")
    # ax2.yaxis.tick_right()

    # plt.show()

    ax3 = plot_mean_vs_std(ax3, distance, mean_orig, color_orig, line_width)
    ax3 = plot_mean_vs_std(ax3, distance, mean_rep, color_lay3, line_width)
    ax3.set_ylabel("Deg. of violation", fontsize=font_size)
    ax3.set_xlabel(
        "$L_2$-distance to the nearest neighbor", fontsize=font_size
    )
    ax3.grid(alpha=0.8, linestyle="dashed")
    ax3.tick_params(axis="both", labelsize=font_size)
    xlim = [0, 2.0]
    ylim = [-0.1, 1.0]
    ax3.set_xlim(xlim)
    ax3.set_ylim(ylim)
    ax3.set_yticks(np.linspace(ylim[0], ylim[1], 3, endpoint=True))
    ax3.set_xticks(np.linspace(xlim[0], xlim[1], 4, endpoint=True))
