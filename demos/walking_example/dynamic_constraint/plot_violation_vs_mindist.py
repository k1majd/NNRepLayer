...
def plot_mean_vs_std(ax, distance, mean, lim_uc, color):
    ax.fill_between(
        distance,
        mean(distance),
        lim_uc(distance),
    ...
    ax.plot(distance, mean(distance), color=color)
    return ax
...
if __name__ == "__main__":
    ...
    ax = plot_mean_vs_std(
        ax, dist_lay3, mean_lay3, lim_uc_lay3, color="#173f5f"
    )
    ax = plot_mean_vs_std(
        ax, dist_lay4, mean_lay4, lim_uc_lay4, color="#e66101"
    )
    ax = plot_mean_vs_std(
        ax, dist_orig, mean_orig, lim_uc_orig, color="#cc79a7"
    )
    ax.set_xlabel("Distance to nearest neighbor")
    ax.set_ylabel("Violation")
    ax.set_title("Violation Error Bars")
    plt.show()
