...
if __name__ == "__main__":
    ...
    # load the repaired relaxed dat set
    load_str = "_8_1_2022_13_8_47"
    repaired_layer = 2
    model_repaired_relaxed = keras.models.load_model(
        os.path.dirname(os.path.realpath(__file__))
    ...
    weights_repaired_relaxed = np.concatenate(
        (
            model_repaired_relaxed.get_weights()[
                2 * (repaired_layer - 1)
            ].flatten(),
            model_repaired_relaxed.get_weights()[
                2 * (repaired_layer - 1) + 1
    ...
    err = weights_orig - weights_repaired_relaxed
    num_repaired_weights = 0
    num_matched_idx = 0
    for i in range(err.shape[0]):
        if err[i] > 3e-08:
    ...
    print(
        f"weight l1 norm is {np.linalg.norm(weights_orig - weights_repaired_relaxed, 1)}"
    )
    print(
        f"weight l-inf norm is {np.linalg.norm(weights_orig - weights_repaired_relaxed, np.inf)}"
    )
    print(
        f"number of repaired weights is {num_repaired_weights}/{err.shape[0]}"
    )
    print(f"number of matched weights is {num_matched_idx}/{len(idx)}")
