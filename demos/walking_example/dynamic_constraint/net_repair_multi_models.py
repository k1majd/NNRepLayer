...
def give_stats(model, x, y, y_pred_prev, bound):
    ...
    for i in range(x.shape[0]):
        if np.abs(delta_u_prev[i]) <= bound - 0.2:
            x_temp.append(x[i])
            y_orig.append(y[i])
...
if __name__ == "__main__":
    ...
    options = Options(
        ...
        "keras",
        {
            "timelimit": 600,  # max time algorithm will take in seconds
            "mipgap": 0.10,  #
            "mipfocus": 2,  #
            "cuts": 0,
            "concurrentmip": 3,
            # "threads": 48,
            "improvestarttime": 500,
    ...
    _, mae = give_stats(
        out_model, test_obs, test_ctrls, ctrl_test_pred_orig, bound
    )
    sat_rate, _ = give_stats(
        out_model, test_obs, test_ctrls, ctrl_test_pred_orig, bound + 0.3
    )
    with open(
