...
def select_repair_nodes(model_orig, model_repair_prev, layer_to_repair):
    repair_node_list = []
    err_w = (
        model_orig.get_weights()[2 * (layer_to_repair - 1)]
        - model_repair_prev.get_weights()[2 * (layer_to_repair - 1)]
    )
    node_w = np.where(err_w > 0.001)[1]
    for i in range(node_w.shape[0]):
        if node_w[i] not in repair_node_list:
            repair_node_list.append(node_w[i])
    err_b = (
        model_orig.get_weights()[2 * (layer_to_repair - 1) + 1]
        - model_repair_prev.get_weights()[2 * (layer_to_repair - 1) + 1]
    )
    node_b = np.where(err_b > 0.001)[0]
    for i in range(node_b.shape[0]):
        if node_b[i] not in repair_node_list:
            repair_node_list.append(node_b[i])
    repair_node_list.sort()
    return repair_node_list
