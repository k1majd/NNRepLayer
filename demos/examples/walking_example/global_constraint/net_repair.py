...
def plotTestData(
    ...
    pred_ctrls_orig = original_model(x_test, training=False)
    pred_ctrls_repair = repaired_model(x_test, training=False)
    ...
    idx = np.where((y_test > bound_lower) & (bound_upper > y_test))[0]
    fig.suptitle(
...
def generate_dataset(obs, ctrl, num_samples):
    rnd_pts = list(np.random.choice(obs.shape[0], num_samples))
    x_train = obs[rnd_pts]
    y_train = ctrl[rnd_pts]
    test_idx = [i for i in range(obs.shape[0]) if i not in rnd_pts]
    x_test = obs[test_idx]
    y_test = ctrl[test_idx]
...
if __name__ == "__main__":
    ...
    with open(
        ...
        + f"/data/repair_dataset_{layer_to_repair}{now_str}.pickle",
        "wb",
    ) as data:
