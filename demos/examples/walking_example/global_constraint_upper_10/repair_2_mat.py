...
if __name__ == "__main__":
    ...
    # separate adversarial inputs - outputs exceed bound
    adv_idx = np.where(model_orig.predict(x_train) > bound)[0]
    adv_x_train = x_train[adv_idx]
    adv_y_train = y_train[adv_idx]
    ...
    model_lay3.layers[-1].set_weights(
        [np.array([[1.0, -1.0]]), np.array([-10.5, 10.5])]
    )
