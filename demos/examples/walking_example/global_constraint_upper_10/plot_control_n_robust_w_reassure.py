...
if __name__ == "__main__":
    ...
    # reassure
    model_torch_data = pickle.load(
        open(
            os.path.dirname(os.path.realpath(__file__))
            + "/reassure/model_torch_bound_data.pkl",
            "rb",
        )
    )
    torch_y = model_torch_data[0]
    torch_err = model_torch_data[1]

    reassure_violate_data = pickle.load(
        open(
            os.path.dirname(os.path.realpath(__file__))
            + "/reassure/violation_degree_reassure_bound_10.pkl",
            "rb",
        )
    )
    dist_reassure = reassure_violate_data[0]
    mean_reassure = reassure_violate_data[1]
    ...
    xlim = [75, 247]
    y_test = y_test[xlim[0] : xlim[1]]
    y_pred_orig = y_pred_orig[xlim[0] : xlim[1]]
    y_pred_lay3 = y_pred_lay3[xlim[0] : xlim[1]]
    y_pred_lay4 = y_pred_lay4[xlim[0] : xlim[1]]
    # y_pred_retrain = y_pred_retrain[xlim[0] : xlim[1]]
    # y_pred_fine = y_pred_fine[xlim[0] : xlim[1]]
    torch_y = torch_y[xlim[0] : xlim[1]]
    torch_err = torch_err[xlim[0] : xlim[1]]
    ...
    ax1.axvspan(T * 23, T * 47, color=color_fill, alpha=0.5)
    ax1.axvspan(T * 134, T * 165, color=color_fill, alpha=0.5)
    ax1.axvspan(
        T * 330.7,
        T * 355,
        color=color_fill,
    ...
    # ax1.set_xlim([0, 400])
    ax1.set_ylim([-15.0, 21.2])
    # ax1.axes.xaxis.set_visible(False)
    ax1.grid(alpha=0.2, linestyle="dashed", color="#7F7F7F")
    ax1.set_xticks(
        np.linspace(
            xlim[0] - xlim[0], T * (xlim[1] - xlim[0]), 5, endpoint=True
        )
    )
    ax1.xaxis.set_ticklabels([])
    # ax1.set_xticks(np.linspace(0, 400, 5, endpoint=True))
    ax1.set_yticks(np.linspace(-10, 20, 4, endpoint=True))
    ...
    ax2.axvspan(T * 23, T * 47, color=color_fill, alpha=0.5)
    ax2.axvspan(T * 134, T * 165, color=color_fill, alpha=0.5)
    ...
    ax2.set_xlim([xlim[0] - xlim[0], T * (xlim[1] - xlim[0])])
