...
def merge_adv_n_old_data(x_old, y_old, x_adv, orig_model):
    y_adv = orig_model.predict(x_adv)
    x = np.concatenate((x_old, x_adv), axis=0)
    y = np.concatenate((y_old, y_adv), axis=0)
    return x, y
...
if __name__ == "__main__":
    ...
    # load test samples
    _, _, test_obs, test_ctrls = generateDataWindow(10)
    # load the mdoel and the original repair dataset
    load_str3 = "_5_31_2022_16_35_50"

    model_repaired, dataset_lay3 = generate_model_n_data(load_str3)
    x_train_orig = dataset_lay3[0]
    y_train_orig = dataset_lay3[1]

    # load new_adv_samples.csv file
    direc = os.path.dirname(os.path.realpath(__file__))
    path_read = os.path.join(direc, "data_adv/new_adv_samples.csv")
    x_adv_new = np.genfromtxt(path_read, delimiter=",").T

    # merge the new adv samples with the original repair dataset
    x_train, y_train = merge_adv_n_old_data(
        x_train_orig, y_train_orig, x_adv_new, ctrl_model_orig
    )
    bound_upper = 10
    bound_lower = 30
    ...
    repair_obj = NNRepair(model_repaired)
