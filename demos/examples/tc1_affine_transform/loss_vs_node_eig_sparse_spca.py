...
def get_sensitive_nodes(
    ...
    #         )
    #     )
    return selected_nodes, cost_vec, all_vec, hessian_loss
...
def main(given_comp):
    ...
    # get sensitive nodes
    repair_indices, cost_vec, all_vec, hessian = get_sensitive_nodes(
        objective, init_params, architecture, 1
    )

    if isPD(hessian) == False:
        hessian = nearestPD(hessian)

    k = 2  # columns
    d = 10  # dimensions
    m = 1000  # sample size
    lambda1 = np.array([0.05])
    lambda2 = 1e-3

    # np.random.seed(10)
    # a = np.random.normal(
    #     0, 1, size=(m, d)
    # )  # generate random normal 1000x500 matrix

    fin_sprout = spca(hessian, lambda1, lambda2, k=k, type=1)
    print(
        f"Finite: {fin_sprout['iter']} iterations with final value {fin_sprout['f_manpg']}, sparsity {fin_sprout['sparsity']}, timediff {fin_sprout['time']}."
    )

    print(fin_sprout["loadings"])

    vec1 = fin_sprout["loadings"][:, 0]
    vec2 = fin_sprout["loadings"][:, 1]
