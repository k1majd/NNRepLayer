...
As we used [Pyomo](http://www.pyomo.org) for formulating the optimization, for info on other supported solver we refer to [here](https://pyomo.readthedocs.io/en/stable/solving_pyomo_models.html#supported-solvers). Note that the solver in NNRepLayer should be specified (read examples).
